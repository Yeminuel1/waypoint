...
2. Click **New repository** → name it `waypoint` → **Create repository**.
...
3. Select the `waypoint` repo you just created.
4. Vercel auto-detects Vite — leave the defaults and click **Deploy**.
5. In a minute or two you'll get a free live URL like `waypoint.vercel.app`.
...
- **Custom domain**: both Vercel and Netlify let you add your own domain
  (e.g. `waypoint.com`) for free — you just need to already own the
